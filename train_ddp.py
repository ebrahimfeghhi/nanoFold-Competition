...
Launch with torchrun:
    torchrun --nproc_per_node=N train_ddp.py --config <path> [--track <id>] ...

All train.py flags are supported. BF16 is used by default when CUDA is available.
Checkpointing and logging are rank-0 only. Validation runs on rank-0 only.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--no-bf16", action="store_true", help="Disable BF16 autocast (use FP32).")
...
# ---------------------------------------------------------------------------
# Misc helpers (copied from train.py)
# ---------------------------------------------------------------------------
...
def make_autocast_ctx(device: torch.device, use_bf16: bool):
...
def resume_metadata_mismatches(*, ckpt_obj, submission_entrypoint_sha256, config_sha256, track_id, fingerprint_sha256, n_params):
    expected_pairs = {
...
def main() -> None:
    ...
    if args.reset_run and is_rank_zero:
        ...
        for p in paths.ckpt_dir.glob("ckpt_step_*.pt"):
            p.unlink()
        last = paths.ckpt_dir / "ckpt_last.pt"
        if last.exists():
            last.unlink()
    ...
    log(f"Using device: {device} | world_size={world_size} | bf16={use_bf16}")

    # Build datasets and samplers
    train_ds = make_dataset(cfg, "train", allow_missing=True)
    ...
    val_loader = None
    if is_rank_zero:
        val_ds = make_dataset(cfg, "val", allow_missing=True)
    ...
    # Each rank processes batch_size samples per micro-step; effective batch = batch_size * grad_accum * world_size
    ...
    log(
        ...
        f"crop={crop_size} bf16={use_bf16}"
    )
    ...
    def run_eval() -> Dict[str, float]:
        ...
        raw_model.eval()
        scores, losses, ca_rmsds, atom14_rmsds = [], [], [], []
        scalar_metrics: Dict[str, list[torch.Tensor]] = {}
        foldscore_metric_values = {name: [] for name in FOLDSCORE_COMPONENT_NAMES} if eval_foldscore_components else {}
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="val", leave=False, disable=not show_progress):
                ...
                with make_autocast_ctx(device, use_bf16):
                    out = run_submission_batch(hooks, model=raw_model, batch=batch, cfg=runtime_cfg,
                                              training=False, expose_supervision=True)
                pred_ca = out["pred_ca"]
                scores.append(batch_lddt_ca(pred_ca, batch["ca_coords"], batch["ca_mask"], batch["residue_mask"]).detach().cpu())
                ca_rmsds.append(batch_rmsd_ca(pred_ca, batch["ca_coords"], batch["ca_mask"], batch["residue_mask"]).detach().cpu())
                atom14_rmsds.append(batch_rmsd_atom14(out["pred_atom14"], batch["atom14_positions"], batch["atom14_mask"], batch["residue_mask"]).detach().cpu())
                if "loss" in out:
                ...
                if eval_foldscore_components:
                    for bidx in range(out["pred_atom14"].shape[0]):
                        comps = foldscore_components(pred_atom14=out["pred_atom14"][bidx],
    ...
    while step < max_steps:
        ...
        if grad_clip and grad_clip > 0:
            grad_norm = float(nn.utils.clip_grad_norm_(raw_model.parameters(), grad_clip).item())
        else:
        ...
        # Aggregate sample counts across ranks
        counts = torch.tensor([samples_this_step, cropped_residues_this_step, nonpad_residues_this_step],
        ...
        step_record = {
            ...
            "nonpad_residue_budget_fraction": (cumulative_nonpad_residues_seen / float(residue_budget)) if residue_budget > 0 else float("nan"),
        }
        ...
        should_log = (step == start_step + 1 or steps_completed_this_run <= 5 or step % log_every == 0 or step == max_steps)
        if is_rank_zero:
            ...
            if should_log:
                ...
                log_line(
                    ...
                    f"loss={train_loss:.4f} lr={lr:.3e} grad_norm={grad_norm:.4f} "
        ...
        if step % eval_every == 0 or step == max_steps:
            ...
            if is_rank_zero:
                ...
                val_metrics["sample_budget_fraction"] = (cumulative_samples_seen / float(sample_budget)) if sample_budget > 0 else float("nan")
